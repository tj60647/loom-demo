...
import LoomMark from "@/components/ui/LoomMark"
  ...
  return (
    <>
      ...
      <JourneyNav
        ...
        // ARE the tabs.
        active={STATION_OF[activeTab]}
        /* In reading focus this bar is the only chrome standing, so it picks up
           the two things the header was carrying: the mark at the left, and the
           header's own items behind one ☰ at the right. Both are withheld
           everywhere else — the header is up there drawing them itself, and a
           second copy would be two doors to each room. */
        mark={readingFocus ? <LoomMark /> : undefined}
        menu={readingFocus ? <ChromeMenu inSandbox={practice} /> : undefined}
