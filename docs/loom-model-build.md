...
5. **Knowledge Graph** — select a Cloth; project it as **the list** (an ordering) and **the board** (a layout: cards in tier bands, threads drawn); per-Projection Tiers, Titles, One-lines, and Descriptions; Unlabeled Passages as unattached group; create Links in-view; export a Projection. **The Capture Log lives here, scoped to this Reading** (TJ, 2026-08-10 — amending tabs 2 and 3 above, which sited it in Reading and Linking). *(Future: Quilts arrive in this same space.)*
