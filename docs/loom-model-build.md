...
- **The whole weave** — **removed from the app 2026-08-11** (the project owner: it "should not be in the app as an idea until the faculty and the authors of the app agree on what it means to have a 'full weave'"). No station, no route, no student surface; rows already written at `scopeKey ''` stay in the database and render nowhere. Reintroduce only with a definition.
...
- Renders in the Library as a **Reading card** with **exactly one door** (ratified 2026-08-08, the project owner):
...
*Ratified 2026-08-08 (the project owner). The sentence the rest of §2 hangs on:*
...
- **Cardinality — ratified 2026-08-08 (the project owner):**
...
- **A Link may precede its Threads** (the project owner, 2026-08-10) — coined in Vocabulary with a gloss, counted at zero, found by search, offered as a chip. The missing symmetry with "a Concept may precede its evidence," now present.
...
- **Every act records the Reading it happened in** (the project owner, 2026-08-11): by the act's own stamp, by its scope, or — for pre-stamp rows — by where the evidence is. Nothing is ever rewritten; what a Reading shows of the record can grow as evidence arrives.
- **Surfaces on 03 · Knowledge Graph, scoped to the open Reading** (the project owner, 2026-08-10) — "the record" beside "the cloth" — and downloads there. (01's "Your work" sheet shows the work itself, as cards; the Log is the record of its making.)
...
(Plain activity names in navigation; the weaving metaphor — Cloth, Thread, Quilt, Join — lives in object names only. The internal route keys keep the July names — `readings · open · throw · map · read` — so `?tab=` never breaks; the visible labels are these five. 03/04 were swapped 2026-08-08, the project owner: the graph before the words.)
...
**The chrome:** the header carries the **guide** — the practice loom at `/sandbox`: the real workbench on a real reading, opening with a worked cloth (nothing is written, by construction; a reload restores the example) and walked in eight beats — *arrive · capture · name · thread · project · sort · board · kit* — each ticked by the student's own act, never by Next; and a **menu** (the project owner, 2026-08-17) holding **My Loom** — a mirror and an exit: counts, the work grouped by the Reading that holds it (as doors), and **start over** (type-to-arm; it has no download button and must not grow one — downloads happen at the object) — and **About**. Workflows left the header the same day: staff reach `/workflows` from the journey bar's staff group, and a student who types the URL still reads their own flow only, decided server-side; there is no student link. (The old first-run "?" walkthrough is gone — the guide is the tutorial.) **Inside a reading the header stands down** — the journey bar carries the save light and the contextual search (§2 Search), and a house icon beside the search is the way back out to where the chrome is (the project owner, 2026-08-19).

*Several modes of reading are still expected inside 01 in future (the project owner); the margin rail and the canvas are the first of that family.*
...
5. The student's work is never inaccessible or partial: whole-artifact export always available ("whole-artifact" is read **by object** — each object exports whole where it lives; the project owner, 2026-08-10); nothing lost on refresh.
