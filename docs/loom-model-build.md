...
- **A Concept may precede its evidence** (ratified 2026-08-08, TJ). Naming an idea you expect to meet — and glossing what you take it to mean — before any Passage supports it is a first-class move, not an oddity: you name it, then read for support. So zero Passages is reached three ways, all legal and indistinguishable in the data:
  ...
  A Concept with no Passages belongs to the Reading it was **named in** (`mintedInSourceId`) until a Passage evidences it somewhere — then membership is the evidence trail, as for any other Concept. Named with no Reading on the act (the stamp is null) it is in scope **everywhere**, the previous empty-evidence rule, kept for the handful of unstamped rows. "No evidence" is a designation, never a warning to act on: the tool counts, it does not judge (red lines 1 and 7).
