...
**Revision history.** *29d* — reflects tool v14 and the July 28–29 calls; §6 splits the artifact from view state; §5 records how the OCR quality gate is met; §4 red line #6 gains a ratified, bounded exception for library ingest. *29e (the project owner)* — §6 adds the graph **development history**, an append-only record of the student's own acts, exploratory only, and an optional capture-provenance `anchor` on exported bytes; both production-build only. *30a (the project owner)* — §3 Global moves export / import / reset onto their own tab (**05 — Keep**) with instructions for each; §6 `views.cardTable` gains `order`, the student's chosen sequence for the sort list. *30b (the project owner)* — §4 red line #6 gains a second ratified exception: model-drafted reading metadata on the Readings tab, admissible only as a proposal an instructor reads and accepts before it is stored (§5). *30c (the project owner)* — **the reading becomes the entry point**: §3 restructures around a shelf of readings, each opening its own workbench; §2 records that a reading is a door into one graph and never re-homes a concept; §6 adds `scopeKey`, reference-only sources, and the card table's `pins`. Also **proposed, not yet ratified**: map **passes**, which move a tier off the concept and onto the pass it belongs to (§3 Map, §6). The reasoning for all of it is in [reading-scope-and-map-passes.md](reading-scope-and-map-passes.md).
...
#### Maps — ratified the project owner, 31 July 2026
...
### Production v1 (the project owner build — ratified 7/28–29)
...
This spec lives in the repo (the project owner's GitHub) beside the code, with a changelog; it is the build target and the release-gate reference.
