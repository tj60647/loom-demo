...
- ~~**The practice loom shows no worked cloth**~~ — **built 2026-08-11.** TJ:
  *"the guide should always be available, like the tutorials in any game. if
  ...
  You'll Go!"* Both halves shipped: a **permanent door in the header**, on
  every page (it was reachable only by typing the URL, and then only from a
  Library card that shows on an empty loom — so the one student who could
  find it was the one who had not started), and a **worked cloth**
  ([practiceCloth.ts](../src/lib/practiceCloth.ts)) built from real
  substrings of that reading's own text layer at their true offsets: four
  passages (one unlabeled), three concepts, two threads (one labelled, one
  left as a description), a Link with its own gloss, a cloth, and a
  projection with tiers and a board.
  It is built on the SERVER and handed down as a prop, because
  `SandboxLoomProvider` must never read a database — `check-sandbox.ts` now
  asserts that seam too, along with the header door. Quotations that stop
